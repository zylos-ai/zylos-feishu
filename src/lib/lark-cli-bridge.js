/**
 * lark-cli bridge — runtime helper for invoking `lark-cli` from zylos-feishu.
 *
 ...
 *     (e.g. calendar_user_login_required).
 */
...
export async function notifyOwnerAuthRequired(err, opts = {}) {
  ...
  if (!ownerOpenId) {
    try {
      ...
      // Config not loadable — probably running outside the zylos-feishu runtime.
    }
  }
  if (!ownerOpenId) {
